...
S = zeros(D,D,K);
  ...
  for t=2:T
    alpha(t,:) = (alpha(t-1,:)*A).*px_z(t,:);                    %SLOW - 49 s for DS20220221 movie 13  - vectorize all of these slow calcs?  %alpha = 600x3 double, A = 3x3 double, px_z = 600x3, each row of alpha is multiplied by A to get a new row
    scale(t) = sum(alpha(t,:));                                  %SLOW - 25 s               %scale = 600x1 double
    alpha(t,:) = alpha(t,:)/scale(t); %nxk matrix                %SLOW - 14.5 s 
    % can't vectorize line 61 (or most of these operations) -- dependent on
    % output from previous loop iteration
  end
  
  % Backward pass (with scaling)
  beta(T,:) = ones(1,K)/scale(T);
  for t=T-1:-1:1
    beta(t,:) = (beta(t+1,:).*px_z(t+1,:))*A'/scale(t);          %SLOW -49.8 s
  end
  
  % Another pass gives us the joint probabilities
  for t=1:T-1
      Xi=Xi+A.*(alpha(t,:)'*(beta(t+1,:).*px_z(t+1,:)));         %SLOW - 68 s
  end
