...
[filename, path] = uigetfile('*traces.dat;*traces.mat;*.traces; *.json; *.json.zip','Please select your traces files.','Multiselect','on');
...
for n=1:nfiles
    ...
    elseif strcmp(filepath(end-4:end),'.json') || strcmp(filepath(end-8:end),'.json.zip')
        traces2ch = openOpenFRET(filepath,options.donor_crosstalk);
        traces = extractChannel(traces2ch,options.channel);
